          ...
          el-collapse-item(title='Tu reserva', name='1')
            ...
            el-form-item(label='Salón')
              el-select(v-model='data.bookingSalon', placeholder='Seleccione un salón', @change="changeBookingSalon")
                el-option(v-for='item in this.$store.state.Site.bookingSalons.beautySalons', :key='item.id', :label='item.name', :value='item.id')
            ...
            el-form-item(label='Servicios', v-if="data.bookingHour")
              el-select(v-model='data.bookingService', placeholder='Seleccione un servicio')
                el-option(v-for='item in this.$store.state.Site.bookingSalons.beautyServices', :key='item.id', :label='item.name', :value='item.id')
            el-form-item
...
export default Vue.extend({
  ...
  computed: {
    ...
    summary(): any {
      ...
      if (this.data.bookingSalon) {
        bookingSalon = this.$store.state.Site.bookingSalons.beautySalons.find(
          (x: any) => x.id === this.data.bookingSalon
        ).name;
      }
      let bookingService: string = "";
      if (this.data.bookingService) {
        bookingService = this.$store.state.Site.bookingSalons.beautyServices.find(
  ...
  methods: {
    ...
    async changeBookingSalon() {
      ...
      const beautySalon = this.$store.state.Site.bookingSalons.beautySalons.find(
        (x: any) => x.id === this.data.bookingSalon
      );
    ...
    async confirmReservation() {
      ...
      try {
        await axios.post(
          ...
          {
            operation: "post beautyReservations",
            param: {
              data: {
                id: uuidv4(),
